...
function chartmake(nameput, dataput, type) {
...
$('document').ready(function() {
    $.get("php/login.php", "login", function(data) {
        if (data === 'success') {
            var logged = true;
            $(".add-button-style .button-value").show();
            $("#icon-image-border").hide();
    ...
    $(".graphopen").click(function() {
        ...
        $(".add-" + type + "-info").slideToggle(400);
        $(".add-" + type + "-info").html('<div class ="graphplace" id="graph' + type + '"><form id="yearsel' + type + '"><select id="yearsal' + type + '" name="year"></select></form><canvas id="' + type + 'Chart"></canvas></div>')
        $.get("php/graph.php", "type=" + type + "&y", function(data) {
            $years = JSON.parse(data);
            $.each($years, function(i, value) {
                $("#yearsal" + type).append($("<option>").text((value) - 1).attr("value", value));
                var year = $("#yearsal" + type).val();
                var name = type + " " + (year - 1);
                $.get("php/graph.php", "type=" + type + "&i=" + year, function(data) {
                    var chartdata = (JSON.parse(data));
                    chartmake(name, chartdata, type);
                });
            });
            $("#yearsel" + type).change(function() {
                var year = $("#yearsal" + type).val();
                var name = type + " " + (year - 1);
                $.get("php/graph.php", "type=" + type + "&i=" + year, function(data) {
                    var chartdata = (JSON.parse(data));
    ...
    $(".recordopen").click(function() {
        ...
        $(".add-" + rectype + "-info").slideToggle(400);
        $(".add-" + rectype + "-info").html('<div class="widget-container ' + rectype + 'recordput"></div>')
        $.get("php/records.php", "type=" + rectype, function(data) {
            $("." + rectype + "recordput").html(data);
            $(".justwork").click(function() {
				$("." + rectype + "recordput").html("blah");
